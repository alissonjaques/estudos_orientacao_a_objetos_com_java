 	 ...
 	 */
  	class RoboSimples{ // declaração da classe
  		...
  		RoboSimples(String n,int px,int py,char d){
  			...
  			nomeDoRobô = n;
  			posiçãoAtual = new Ponto2D(px,py);
  			direçãoAtual = d;
  		...
  		public void move(int passos){
  			...
  			if (direçãoAtual == 'N') posiçãoAtual.modificaY(posiçãoAtual.retornaY()+passos);
  			if (direçãoAtual == 'S') posiçãoAtual.modificaY(posiçãoAtual.retornaY()-passos);
  			if (direçãoAtual == 'E') posiçãoAtual.modificaX(posiçãoAtual.retornaX()+passos);
  			if (direçãoAtual == 'O') posiçãoAtual.modificaX(posiçãoAtual.retornaX()-passos);
  		 ...
  		 * sido criado. Caso uma direção inválida seja informada, a direção é mudado para
  		 * o norte.
  		 * @param novaDireção a nova direção para o robô.
  		 */
  		public void mudaDireção(char novaDireção){
  			
  			if(novaDireção == 'N' || novaDireção == 'S' || novaDireção == 'E' || novaDireção == 'O') { // se a direção for válida
  			
  				direçãoAtual = novaDireção; // mude a direção
  			
  			}else { // senão
  				
  				direçãoAtual = 'N'; // mude para a direção norte
  		...
  		public String toString(){
  			...
  			posição = posição+"Posição do robô: "+posiçãoAtual+"\n";
