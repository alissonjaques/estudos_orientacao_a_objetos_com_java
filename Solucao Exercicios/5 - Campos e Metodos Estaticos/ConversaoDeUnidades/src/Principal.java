...
/**
 * 	A classe Principal possuí vários métodos estáticos, incluindo o método main
 * 	para sua execução. Esta classe demonstra vários exemplos de usos de classes
 *  bibliotecas, referentes a conversão de unidades.
...
public class Principal { // declaração da classe
	...
	public static void main(String[] argumentos) {
		...
		final double distanciaDaTerraALuaEmQuilometros = 382000;        // a distância da terra à lua em quilômetros
		final double areaCampoFutebolMetrosQuadrados = 8250;			// a área de um campo de futebol em metros quadrados
		final double volumePiscinaOlimpicaMetrosCubicos = 1890;			// o volume de uma piscina olímpica em metros cúbicos
		final double velocidadeDaLuzEmQuilometrosPorHora = 1080000000;  // a velociadade da luz em km/h
		final double periodoDeGestacaoElefanteIndianoEmDias = 624;		// o período de gestação de um elefante indiano em dias
	...
	/**
	 *  O método gestacaoElefanteIndiano recebe um único argumento,
	 *  referente ao período de gestação em dias, e imprime a conversão
	 *  desse valor em horas, minutos e segundos (imprime também o valor
	 *  em dias). Este método demonstra usos da classe ConversaoDeUnidadesDeTempo 
	 *  e é um método estático, ou seja, pode ser chamado diretamente por classes
	 *  usuárias. Este método é uma subrotina do método main principal dessa classe.
	 *  @param periodoEmDias o tempo de gestação de um elefante indiano em dias
	 * 
	 * */	
